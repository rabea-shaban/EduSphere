...
export class QueryBuilder<T> {
   ...
   */
  filter(filterWhitelist: string[] = []): this {
   ...
   */
  range(field: string, minKey: string, maxKey: string): this {
    const minVal = this.query[minKey];
    const maxVal = this.query[maxKey];
    const rangeFilter: any = {};

    if (minVal !== undefined && minVal !== '') rangeFilter.$gte = Number(minVal);
    if (maxVal !== undefined && maxVal !== '') rangeFilter.$lte = Number(maxVal);
   ...
   */
  sort(sortWhitelist: string[] = [], defaultSort: any = { createdAt: -1 }): this {
    const sortParam = this.query.sort;
    let sortBy: any = defaultSort;
   ...
   * Apply pagination
   */
  paginate(defaultLimit: number = 20): this {
   ...
   */
  async execute(model: Model<any>): Promise<{ data: T[]; pagination: IPaginationMeta }> {
