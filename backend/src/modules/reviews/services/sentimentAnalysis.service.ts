import { ReviewSentiment } from '../review.model';
...
export class SentimentAnalysisService {
   ...
   */
  static analyze(comment: string, rating: number): { sentiment: ReviewSentiment; keywords: string[] } {
    const text = comment.toLowerCase();

    // Default sentiment based on rating
    let sentiment: ReviewSentiment = rating >= 4 ? 'POSITIVE' : rating <= 2 ? 'NEGATIVE' : 'NEUTRAL';
    ...
    if (text.includes('شرح')) keywords.push('جودة الشرح');
    if (text.includes('تمارين') || text.includes('أسئلة')) keywords.push('تطبيقات وأمثلة');
    if (text.includes('صوت') || text.includes('فيديو')) keywords.push('جودة التسجيل');
    if (text.includes('مبسط') || text.includes('سهل')) keywords.push('أسلوب التبسيط');
    if (text.includes('اختبار') || text.includes('واجب')) keywords.push('التقييمات');
