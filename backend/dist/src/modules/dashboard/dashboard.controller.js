...
exports.getDashboardData = (0, catchAsync_1.catchAsync)(async (req, res) => {
    ...
    if (role === 'SUPER_ADMIN' || role === 'ADMIN') {
        ...
        const monthsPromises = monthIndexes.map(async (i) => {
            ...
            const [students, teachers, courses, revAgg] = await Promise.all([
                user_model_1.User.countDocuments({ role: 'STUDENT', createdAt: { $gte: startOfMonth, $lte: endOfMonth } }),
                user_model_1.User.countDocuments({ role: 'TEACHER', createdAt: { $gte: startOfMonth, $lte: endOfMonth } }),
                course_model_1.Course.countDocuments({ createdAt: { $gte: startOfMonth, $lte: endOfMonth } }),
        ...
        const dailyPromises = dayIndexes.map(async (i) => {
            ...
            const [signups, enrollments] = await Promise.all([
                user_model_1.User.countDocuments({ createdAt: { $gte: startOfDay, $lte: endOfDay } }),
                enrollment_model_1.Enrollment.countDocuments({ createdAt: { $gte: startOfDay, $lte: endOfDay } }),
            ]);
            return {
        ...
        const [totalStudents, totalTeachers, totalAdmins, totalUsers, pendingTeacherApps, totalCourses, publishedCourses, pendingCourseReviews, totalQuizzes, activeSubscriptions, paymentRevAgg, enrollmentRevAgg, pendingPayments, withdrawalRequests, monthsData, dailyActivityData, recentTeacherApplications, recentPayments, recentUsers, latestNotifications, unreadNotificationsCount,] = await Promise.all([
            user_model_1.User.countDocuments({ role: 'STUDENT' }),
            user_model_1.User.countDocuments({ role: 'TEACHER' }),
            user_model_1.User.countDocuments({ role: { $in: ['ADMIN', 'SUPER_ADMIN'] } }),
            user_model_1.User.countDocuments({}),
            teacherApplication_model_1.TeacherApplication.countDocuments({ status: { $in: ['Pending', 'UnderReview'] } }),
            course_model_1.Course.countDocuments({}),
            course_model_1.Course.countDocuments({ status: 'Published' }),
            course_model_1.Course.countDocuments({ status: { $ne: 'Published' } }),
            quiz_model_1.Quiz.countDocuments({}),
            enrollment_model_1.Enrollment.countDocuments({ status: 'Active' }),
            ...
            payment_model_1.Payment.countDocuments({ status: 'Pending' }),
            payment_model_1.Payment.countDocuments({ status: 'Pending' }),
            Promise.all(monthsPromises),
            Promise.all(dailyPromises),
            teacherApplication_model_1.TeacherApplication.find({}).sort({ createdAt: -1 }).limit(5).select('fullName subject stage status createdAt experienceYears phone email').lean(),
            payment_model_1.Payment.find({}).sort({ createdAt: -1 }).limit(5).populate('studentId', 'firstName lastName email avatar').populate('courseId', 'title').lean(),
            user_model_1.User.find({}).sort({ createdAt: -1 }).limit(5).select('firstName lastName username email role avatar createdAt').lean(),
            notification_model_1.Notification.find({ recipientId: userId }).sort({ createdAt: -1 }).limit(5).lean(),
        ...
        dashboardData = {
            ...
            systemHealth: {
                ...
                dbStatus: dbStateMap[mongoose_1.default.connection.readyState] || 'Connected',
        ...
        const teacherCourses = await course_model_1.Course.find({ teacher: userId }).lean();
        ...
        const pendingCourses = teacherCourses.filter((c) => c.status === 'Pending' || c.status === 'UnderReview').length;
        ...
        const teacherMonthsPromises = monthIndexes.map(async (i) => {
            ...
            const [mRevAgg, mStudents] = await Promise.all([
                ...
                enrollment_model_1.Enrollment.countDocuments({ courseId: { $in: teacherCourseIds }, createdAt: { $gte: mStart, $lte: mEnd } }),
            ]);
        ...
        const [totalStudents, totalLessons, totalQuizzes, totalAssignments, certificatesIssued, revenueAgg, monthlyRevAgg, lastMonthRevAgg, todayRevAgg, pendingRevAgg, rawMonthsChartData, recentEnrollmentsRaw, latestNotifications, unreadNotificationsCount,] = await Promise.all([
            enrollment_model_1.Enrollment.countDocuments({ courseId: { $in: teacherCourseIds } }),
            lesson_model_1.Lesson.countDocuments({ courseId: { $in: teacherCourseIds } }),
            quiz_model_1.Quiz.countDocuments({ courseId: { $in: teacherCourseIds } }),
            assignment_model_1.Assignment.countDocuments({ courseId: { $in: teacherCourseIds } }),
            enrollment_model_1.Enrollment.countDocuments({ courseId: { $in: teacherCourseIds }, isCompleted: true }),
            payment_model_1.Payment.aggregate([{ $match: { courseId: { $in: teacherCourseIds }, status: 'Paid' } }, { $group: { _id: null, total: { $sum: '$amount' } } }]),
            payment_model_1.Payment.aggregate([{ $match: { courseId: { $in: teacherCourseIds }, status: 'Paid', createdAt: { $gte: startOfMonth, $lte: endOfMonth } } }, { $group: { _id: null, total: { $sum: '$amount' } } }]),
            payment_model_1.Payment.aggregate([{ $match: { courseId: { $in: teacherCourseIds }, status: 'Paid', createdAt: { $gte: startOfLastMonth, $lte: endOfLastMonth } } }, { $group: { _id: null, total: { $sum: '$amount' } } }]),
            payment_model_1.Payment.aggregate([{ $match: { courseId: { $in: teacherCourseIds }, status: 'Paid', createdAt: { $gte: startOfToday } } }, { $group: { _id: null, total: { $sum: '$amount' } } }]),
            payment_model_1.Payment.aggregate([{ $match: { courseId: { $in: teacherCourseIds }, status: 'Pending' } }, { $group: { _id: null, total: { $sum: '$amount' } } }]),
            Promise.all(teacherMonthsPromises),
            enrollment_model_1.Enrollment.find({ courseId: { $in: teacherCourseIds } })
                .sort({ createdAt: -1 })
                .limit(6)
        ...
        const revenueGrowth = lastMonthRevenue > 0
            ...
            : 12.5;
        const recentStudents = recentEnrollmentsRaw.map((enr) => {
        ...
        recentEnrollmentsRaw.slice(0, 3).forEach((enr) => {
            const student = enr.studentId || {};
            const course = enr.courseId || {};
            const fullName = (student.firstName || student.lastName)
                ? `${student.firstName || ''} ${student.lastName || ''}`.trim()
        ...
        teacherCourses.filter((c) => c.status === 'Published').slice(0, 2).forEach((c) => {
            recentActivities.push({
        ...
        dashboardData = {
            ...
            analytics: {
                courseViews: teacherCourses.reduce((acc, c) => acc + (c.views || 120), 0),
        ...
        const [studentEnrollments, latestNotifications, unreadNotificationsCount] = await Promise.all([
            enrollment_model_1.Enrollment.find({ studentId: userId }).populate('courseId', 'title thumbnail category teacher level').lean(),
            notification_model_1.Notification.find({ recipientId: userId }).sort({ createdAt: -1 }).limit(5).lean(),
            notification_model_1.Notification.countDocuments({ recipientId: userId, isRead: false }),
        ]);
        const activeEnrollments = studentEnrollments.filter((e) => e.status === 'Active').length;
        const completedCourses = studentEnrollments.filter((e) => e.status === 'Completed' || Boolean(e.completedAt)).length;
        dashboardData = {
            welcome: {
    ...
    res.status(200).json(new ApiResponse_1.ApiResponse(200, dashboardData, 'تم جلب بيانات لوحة التحكم بنجاح'));
});
