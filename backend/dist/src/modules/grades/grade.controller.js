...
exports.createGrade = (0, catchAsync_1.catchAsync)(async (req, res) => {
    ...
    if (name?.ar || name?.en) {
        ...
        if (name?.ar)
            nameConditions.push({ 'name.ar': name.ar });
        if (name?.en)
            nameConditions.push({ 'name.en': name.en });
        const existingName = await grade_model_1.Grade.findOne({ $or: nameConditions });
        if (existingName) {
            if (name?.ar && existingName.name.ar === name.ar) {
                throw new ApiError_1.ApiError(400, 'اسم الصف أو المسار باللغة العربية موجود بالفعل');
            }
            if (name?.en && existingName.name.en === name.en) {
                throw new ApiError_1.ApiError(400, 'اسم الصف أو المسار باللغة الإنجليزية موجود بالفعل');
            }
        }
    }
    // Auto-resolve order conflict or auto-assign next unique order
    const reqOrder = Number(order);
    const existingOrder = await grade_model_1.Grade.findOne({ order: reqOrder });
    if (!reqOrder || isNaN(reqOrder) || existingOrder) {
        const highestGrade = await grade_model_1.Grade.findOne().sort({ order: -1 }).select('order').lean();
    ...
    res.status(201).json(new ApiResponse_1.ApiResponse(201, grade, 'تم إضافة المسار أو الصف الدراسي بنجاح'));
});
/**
...
exports.getAllGrades = (0, catchAsync_1.catchAsync)(async (req, res) => {
    ...
    const gradesRaw = await grade_model_1.Grade.find(filter)
        .sort(sortBy)
        .skip(skip)
        .limit(limitNum)
        .lean();
    ...
    const grades = await Promise.all(gradesRaw.map(async (g) => {
        const [subjectsCount, coursesCount] = await Promise.all([
            subject_model_1.Subject.countDocuments({ grades: g._id }),
            course_model_1.Course.countDocuments({ grade: g._id }),
        ]);
        return {
            ...g,
            subjectsCount,
...
exports.updateGrade = (0, catchAsync_1.catchAsync)(async (req, res) => {
    ...
    const grade = await grade_model_1.Grade.findById(id);
    if (!grade) {
        throw new ApiError_1.ApiError(404, 'الصف أو المسار غير موجود');
    }
    const { name, order, educationStage, description, isActive } = req.body;
    if (name?.ar && name.ar !== grade.name.ar) {
        const dupAr = await grade_model_1.Grade.findOne({ 'name.ar': name.ar, _id: { $ne: id } });
        if (dupAr)
            throw new ApiError_1.ApiError(400, 'اسم الصف/المسار باللغة العربية مكرر لصف آخر');
    }
    if (name?.en && name.en !== grade.name.en) {
        const dupEn = await grade_model_1.Grade.findOne({ 'name.en': name.en, _id: { $ne: id } });
        if (dupEn)
            throw new ApiError_1.ApiError(400, 'اسم الصف/المسار باللغة الإنجليزية مكرر لصف آخر');
    }
    let finalOrder = grade.order;
    if (order !== undefined && order !== null && Number(order) !== grade.order) {
        const reqOrder = Number(order);
        const dupOrder = await grade_model_1.Grade.findOne({ order: reqOrder, _id: { $ne: id } });
        if (dupOrder) {
            const highestGrade = await grade_model_1.Grade.findOne().sort({ order: -1 }).select('order').lean();
    ...
    await grade.save();
    res.status(200).json(new ApiResponse_1.ApiResponse(200, grade, 'تم تحديث الصف/المسار الدراسي بنجاح'));
});
/**
 * Delete Grade.
 */
