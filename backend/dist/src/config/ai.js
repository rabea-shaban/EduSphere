...
class OpenAiProvider {
    ...
    async generateJson(prompt, systemMessage) {
        const messages = [];
        if (systemMessage)
            messages.push({ role: 'system', content: systemMessage + ' Respond in JSON format only.' });
        messages.push({ role: 'user', content: prompt });
        const response = await this.requestOpenAi({
            model: this.model,
            messages,
    ...
    async moderateText(input) {
        ...
        try {
            const result = await this.generateJson(`Determine if the following content is inappropriate, spam, or contains offensive language. Respond ONLY with a JSON object: { "flagged": boolean }.\n\nContent: "${input}"`, 'You are a moderation tool.');
...
class MockAiProvider {
    async generateText(prompt, _systemMessage) {
        let mockResponse = '';
...
- **Saturated Hydrocarbon**: Hydrocarbons containing only single covalent bonds.`;
    ...
    async generateJson(prompt, _systemMessage) {
        const lower = prompt.toLowerCase();
        let resObj = {};
...
exports.aiProvider = new AiProviderFactory().getProvider();
exports.default = exports.aiProvider;
