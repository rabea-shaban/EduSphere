...
import {
  useTeacherEarningsDashboard,
  useTeacherTransactions,
} from "@/hooks/useTeacherEarnings";
...
import { PrintableFinancialStatement } from "@/features/teacher/components/earnings/printable-financial-statement";

export default function InstructorEarningsPage() {
  const [showWithdrawModal, setShowWithdrawModal] = React.useState(false);

  const { data: dashboard, isLoading, refetch } = useTeacherEarningsDashboard();
  const { data: transactionsData } = useTeacherTransactions({ limit: 100 });
  ...
  return (
    <>
      {/* On Screen View */}
      <div className="space-y-6 sm:space-y-8 text-right dir-rtl max-w-6xl mx-auto pb-12 print:hidden">
        {/* Top Banner Header */}
      ...
      {/* Enterprise Printable Financial Statement Sheet */}
      <PrintableFinancialStatement dashboard={dashboard} transactions={transactions} />
    </>
  );
}
