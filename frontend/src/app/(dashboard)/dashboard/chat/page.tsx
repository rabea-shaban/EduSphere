...
export default function StudentChatPage() {
  ...
  const {
    ...
    storageKey: "student_last_conv_id",
  ...
  const isTargetOnline = React.useMemo(() => {
    if (!activeOtherParticipant?._id) return false;
    if (isUserOnline(activeOtherParticipant._id)) return true;
    if ((activeOtherParticipant as any)?.lastActiveAt) {
      const diff = Date.now() - new Date((activeOtherParticipant as any).lastActiveAt).getTime();
      return diff < 60000; // Online if active in last 60 seconds
    }
    return false;
  }, [activeOtherParticipant, isUserOnline]);
  ...
  const handleBlockUser = async () => {
    if (!activeOtherParticipant?._id) return;
    if (confirm("هل أنت تأكد من حظر هذا المستخدم؟")) {
      try {
        await api.patch(`/users/${activeOtherParticipant._id}`, { isBlocked: true });
  ...
  return (
    <div className="h-[calc(100vh-120px)] md:h-[calc(100vh-100px)] flex flex-col md:flex-row rounded-3xl bg-white dark:bg-[#071C3B] border border-slate-200/80 dark:border-white/10 shadow-xl overflow-hidden text-right" dir="rtl">
      ...
      {/* ========================================================== */}
      {/* 1. RIGHT SIDEBAR: CONVERSATIONS LIST & GLOBAL USER SEARCH */}
      {/* ========================================================== */}
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1 scrollbar-thin">
          {isLoadingConversations ? (
            ...
            <div className="text-center py-12 px-4 space-y-3">
              ...
              <p className="text-xs font-bold text-slate-500">لا توجد محادثات مطابقة</p>
              <p className="text-[11px] text-slate-400">ابحث أعلاه برقم الهاتف أو الاسم أو قم بإنشاء جروب جديد</p>
            </div>
          ) : (
          ...
          <>
            ...
            <div className="p-4 border-b border-slate-200/80 dark:border-white/10 flex items-center justify-between bg-slate-50/50 dark:bg-black/20">
              <div className="flex items-center gap-3">
                ...
                <div className="relative">
                  {isGroup ? (
                    <div className="h-10 w-10 rounded-xl bg-gradient-to-tr from-purple-600 to-[#1E73D8] text-white flex items-center justify-center font-black">
                      <Users className="h-5 w-5" />
                    </div>
                  ) : activeOtherParticipant?.avatar ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={activeOtherParticipant.avatar} alt="User" className="h-10 w-10 rounded-xl object-cover border border-[#F58220]/40" />
                  ) : (
                    <div className="h-10 w-10 rounded-xl bg-gradient-to-tr from-[#0B2D5B] to-[#1E73D8] text-white flex items-center justify-center font-black">
                      {activeOtherParticipant?.firstName?.charAt(0) || "U"}
                    </div>
                  )}

                  {!isGroup && (
                    <div className="relative">
                      <span className={`absolute -bottom-0.5 -right-0.5 h-3 w-3 rounded-full border-2 border-white dark:border-[#071C3B] ${isTargetOnline ? "bg-emerald-500" : "bg-slate-400"}`} />
                    </div>
                  )}
                </div>
                ...
                <div>
                  <h3 className="text-sm font-black text-[#0B2D5B] dark:text-white flex items-center gap-1.5">
                    <span>
                      {isGroup
                        ...
                        : `${activeOtherParticipant?.firstName || ""} ${activeOtherParticipant?.lastName || ""}`.trim() || activeOtherParticipant?.username}
                    </span>
                    {!isGroup && <ShieldCheck className="h-4 w-4 text-[#F58220]" />}
                  </h3>
                  ...
                  {isGroup ? (
                    ...
                    <span className={`text-[11px] font-bold flex items-center gap-1.5 ${isTargetOnline ? "text-emerald-600 dark:text-emerald-400" : "text-slate-400"}`}>
                      <span className={`h-2 w-2 rounded-full ${isTargetOnline ? "bg-emerald-500 animate-pulse" : "bg-slate-400"}`} />
                      <span>{isTargetOnline ? "متصل الآن" : "غير متصل"}</span>
                    </span>
                  )}
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 relative">
                {!isGroup && activeOtherParticipant && activeConversation?._id && (
                  <>
                    <Button
                      variant="outline"
                      size="icon"
                      onClick={() => {
                        const targetId = activeOtherParticipant?._id;
                        const convId = activeConversation?._id;
                        if (!targetId || !convId) return;

                        const recipientName = `${activeOtherParticipant.firstName || ""} ${activeOtherParticipant.lastName || ""}`.trim() || activeOtherParticipant.username || "مستخدم المنصة";
                ...
                <span className="text-xs font-bold text-[#1E5DB8] bg-[#EAF3FF] dark:bg-white/10 px-3 py-1 rounded-full border border-[#E5EAF2] dark:border-white/10 hidden sm:inline-block">
                  {isGroup ? "مجموعة دراسية 👥" : activeOtherParticipant?.role === "TEACHER" ? "مدرس مادة معتمد" : "مستخدم في المنصة"}
                </span>
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 scrollbar-thin bg-slate-50/80 dark:bg-[#041329]/80">
              {isLoadingMessages ? (
                ...
                <div className="text-center py-20 space-y-3">
                  <Sparkles className="h-10 w-10 text-[#F58220] mx-auto animate-pulse" />
                  <h4 className="text-sm font-bold text-[#0B2D5B] dark:text-white">بدء المحادثة المباشرة</h4>
                  <p className="text-xs text-slate-500 max-w-sm mx-auto">
                    اكتب سؤالك أو استفسارك وسيصل الرد فورياً.
                  </p>
                </div>
              ) : (
                messages.map((msg, idx) => {
                  ...
                  // ─── Pure Audio message: render premium AudioMessage card ─────
              ...
              {isTyping && (
                <div className="flex items-center gap-2 text-xs font-bold text-[#F58220] animate-pulse">
            ...
            <form onSubmit={handleSendMessage} className="p-3 sm:p-4 border-t border-slate-200/80 dark:border-white/10 bg-white dark:bg-[#071C3B] flex items-center gap-2">
              ...
              {/* Voice Recorder — expands to fill row when active */}
              <VoiceRecorderButton
                disabled={isUploading}
