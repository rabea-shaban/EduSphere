...
function FeatureIcon({ name }: { name: FeatureBarItem["icon"] }) {
  const cls = "h-6 w-6 text-[#1E73D8] shrink-0";
  switch (name) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-10 py-8">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {items.map((item, idx) => (
            ...
            >
              {/* Icon circle */}
              <div className="shrink-0 flex items-center justify-center w-12 h-12 rounded-2xl bg-[#EBF4FF]">
                <FeatureIcon name={item.icon} />
              </div>
              {/* Text */}
              <div>
                <p
                  className="text-sm font-bold text-[#0B2D5B] leading-tight"
                  style={{ fontFamily: "'Cairo', sans-serif" }}
                >
                  {item.title}
                </p>
                <p
                  className="text-[11px] text-[#64748B] font-medium leading-tight mt-0.5"
                  style={{ fontFamily: "'Cairo', sans-serif" }}
                >
                  {item.description}
                </p>
