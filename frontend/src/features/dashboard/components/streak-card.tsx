...
export function StreakCard({ profile }: StreakCardProps) {
  return (
    <motion.div
      ...
      className="relative rounded-3xl p-6 bg-gradient-to-br from-[#0B2D5B] via-[#071C3B] to-[#0F274D] text-white overflow-hidden shadow-xl shadow-[#0B2D5B]/20 text-right"
    >
      ...
      <div className="relative z-10 flex flex-col justify-between h-full space-y-5">
        ...
        <div className="space-y-2">
          <div className="flex justify-between text-xs font-bold">
            ...
            <span className="text-[#FF9A2A] font-extrabold">{profile.xpPoints} / 4000 XP</span>
          </div>
          <div className="w-full bg-white/10 h-2.5 rounded-full overflow-hidden p-0.5 border border-white/10">
            <motion.div
              initial={{ width: 0 }}
              animate={{ width: `${(profile.xpPoints / 4000) * 100}%` }}
        ...
        <div className="flex items-center justify-between text-[11px] text-blue-100 font-semibold pt-1 border-t border-white/10">
          <span className="flex items-center gap-1">
            ...
            <span>باقي 550 XP للوصول للمستوى 13</span>
          </span>
