...
import { useArchiveAssignment } from "@/hooks/useAssignments";
import type { ApiAssignment } from "@/features/teacher/types/assignment";

interface ArchiveAssignmentDialogProps {
  assignment: ApiAssignment | null;
  isOpen: boolean;
  onClose: () => void;
}

export function ArchiveAssignmentDialog({ assignment, isOpen, onClose }: ArchiveAssignmentDialogProps) {
  const archiveAssignment = useArchiveAssignment();

  const handleConfirm = async () => {
    if (!assignment) return;
    await archiveAssignment.mutateAsync(assignment._id);
    onClose();
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
      ...
      <div className="relative w-full max-w-md bg-white dark:bg-[#0B2D5B] rounded-3xl shadow-2xl border border-slate-200 dark:border-white/10 p-6 space-y-5 text-right">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-base font-black text-[#0B2D5B] dark:text-white">أرشفة الواجب</h2>
            <p className="text-xs text-slate-500 dark:text-slate-400">سيتم نقل الواجب للأرشيف</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 pt-1">
          ...
          >
            {archiveAssignment.isPending ? (
              <Loader2 className="h-4 w-4 animate-spin" />
            ) : (
              <Archive className="h-4 w-4" />
            )}
            <span>{archiveAssignment.isPending ? "جاري الأرشفة..." : "أرشفة الواجب"}</span>
          </button>
        </div>
      </div>
    </div>
...
export default ArchiveAssignmentDialog;
