...
import { useDuplicateAssignment } from "@/hooks/useAssignments";
...
export function DuplicateAssignmentDialog({ assignment, isOpen, onClose }: DuplicateAssignmentDialogProps) {
  const duplicateAssignment = useDuplicateAssignment();

  const handleConfirm = async () => {
    if (!assignment) return;
    await duplicateAssignment.mutateAsync(assignment._id);
    onClose();
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
      ...
      <div className="relative w-full max-w-md bg-white dark:bg-[#0B2D5B] rounded-3xl shadow-2xl border border-slate-200 dark:border-white/10 p-6 space-y-5 text-right">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-base font-black text-[#0B2D5B] dark:text-white">تكرار الواجب التطبيقي</h2>
            <p className="text-xs text-slate-500 dark:text-slate-400">سيتم تكرار الواجب وإعداداته كمسودة</p>
          </div>
        </div>

        <div className="p-4 rounded-2xl bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 space-y-1">
          <p className="text-xs font-bold text-slate-500">الواجب المراد تكراره:</p>
        ...
        <div className="flex items-center gap-3 pt-1">
          ...
          <button
            ...
            disabled={duplicateAssignment.isPending}
          ...
          >
            {duplicateAssignment.isPending ? (
              <Loader2 className="h-4 w-4 animate-spin" />
            ) : (
              <Copy className="h-4 w-4" />
            )}
            <span>{duplicateAssignment.isPending ? "جاري التكرار..." : "تكرار الواجب"}</span>
          </button>
        </div>
      </div>
    </div>
