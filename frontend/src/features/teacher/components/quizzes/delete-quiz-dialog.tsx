...
export function DeleteQuizDialog({ quiz, isOpen, onClose }: DeleteQuizDialogProps) {
  const deleteQuiz = useDeleteQuiz();

  const handleConfirm = async () => {
    if (!quiz) return;
    await deleteQuiz.mutateAsync(quiz._id);
    onClose();
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
      ...
      <div className="relative w-full max-w-md bg-white dark:bg-[#0B2D5B] rounded-3xl shadow-2xl border border-slate-200 dark:border-white/10 p-6 space-y-5 text-right">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-base font-black text-[#0B2D5B] dark:text-white">تأكيد حذف الاختبار</h2>
            <p className="text-xs text-slate-500 dark:text-slate-400">سيتم نقل الاختبار إلى الحذف المؤقت</p>
          </div>
        </div>

        <div className="p-4 rounded-2xl bg-rose-50 dark:bg-rose-900/10 border border-rose-200 dark:border-rose-700/30">
          <p className="text-sm font-bold text-rose-800 dark:text-rose-300">
            هل أنت متأكد من رغبتك في حذف الاختبار:
          </p>
        ...
        <div className="flex items-center gap-3 pt-1">
          ...
          >
            ...
            <span>{deleteQuiz.isPending ? "جاري الحذف..." : "حذف الاختبار"}</span>
          </button>
        </div>
      </div>
    </div>
