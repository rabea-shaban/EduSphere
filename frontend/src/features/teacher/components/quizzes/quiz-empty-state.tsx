...
export function QuizEmptyState({ onCreateClick, filtered }: QuizEmptyStateProps) {
  return (
    <div className="flex flex-col items-center justify-center py-12 text-center gap-3 bg-white dark:bg-[#0F274D] rounded-3xl border border-dashed border-slate-200 dark:border-white/10 p-6">
      ...
      <div className="space-y-1">
        <p className="text-sm font-black text-slate-800 dark:text-slate-100">
          {filtered ? "لا توجد اختبارات تطابق البحث أو الفلتر" : "لا توجد اختبارات تقييمية مضافة حالياً"}
        </p>
        <p className="text-xs text-slate-500 dark:text-slate-400 max-w-sm mx-auto leading-relaxed">
          {filtered
            ? "حاول التغيير في الكلمات الدلالية أو الفلاتر المحددة."
            : "أنشئ أول اختبار تقييمي للطلاب لقياس مدى استيعابهم للمحتوى التعليمي."}
        </p>
      </div>
