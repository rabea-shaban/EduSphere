...
import { useRestoreQuiz } from "@/hooks/useQuizzes";
import type { ApiQuiz } from "@/features/teacher/types/quiz";

interface RestoreQuizDialogProps {
  quiz: ApiQuiz | null;
  isOpen: boolean;
  onClose: () => void;
}

export function RestoreQuizDialog({ quiz, isOpen, onClose }: RestoreQuizDialogProps) {
  const restoreQuiz = useRestoreQuiz();

  const handleConfirm = async () => {
    if (!quiz) return;
    await restoreQuiz.mutateAsync(quiz._id);
    onClose();
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
      ...
      <div className="relative w-full max-w-md bg-white dark:bg-[#0B2D5B] rounded-3xl shadow-2xl border border-slate-200 dark:border-white/10 p-6 space-y-5 text-right">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-base font-black text-[#0B2D5B] dark:text-white">استعادة الاختبار</h2>
            <p className="text-xs text-slate-500 dark:text-slate-400">سيتم استعادة الاختبار كمسودة</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 pt-1">
          ...
          <button
            ...
            disabled={restoreQuiz.isPending}
          ...
          >
            {restoreQuiz.isPending ? (
              <Loader2 className="h-4 w-4 animate-spin" />
            ) : (
              <RotateCcw className="h-4 w-4" />
            )}
            <span>{restoreQuiz.isPending ? "جاري الاستعادة..." : "استعادة الاختبار"}</span>
          </button>
        </div>
      </div>
    </div>
...
export default RestoreQuizDialog;
