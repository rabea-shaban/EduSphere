...
import { X, Loader2, PlayCircle, Upload } from "lucide-react";
...
export function CreateLessonDialog({ sectionId, isOpen, onClose }: CreateLessonDialogProps) {
  const createLesson = useCreateLesson(sectionId);

  const [form, setForm] = React.useState<CreateLessonInput>({
    title: "",
    description: "",
    shortDescription: "",
    content: "",
    ...
    videoUrl: "",
    attachmentUrl: "",
    completionRequirement: "Watch75",
  });

  const [errors, setErrors] = React.useState<Partial<Record<keyof CreateLessonInput, string>>>({});
  ...
  const handleChange = <K extends keyof CreateLessonInput>(
    key: K,
    value: CreateLessonInput[K]
  ) => {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">
                أدخل معلومات الدرس ومحتواه التعلمي
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[70vh] overflow-y-auto text-right">
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              placeholder="مثال: مقدمة في الرياضيات العامة"
          ...
          {(form.lessonType === "Video" || form.lessonType === "Live") && (
            <div className="space-y-1.5">
              <label className="text-xs font-black text-slate-700 dark:text-slate-200">
                رابط الفيديو / البث المباشر (YouTube, Vimeo, HLS)
              </label>
              <input
                type="url"
                value={form.videoUrl || ""}
          ...
          {/* Content (Article or Text) */}
          {(form.lessonType === "Article" || form.lessonType === "Text" || form.lessonType === "Interactive") && (
            <div className="space-y-1.5">
              <label className="text-xs font-black text-slate-700 dark:text-slate-200">
                محتوى الدرس المقالي (نص مفصل / HTML / Markdown)
              </label>
              <textarea
                value={form.content || ""}
                onChange={(e) => handleChange("content", e.target.value)}
                rows={4}
                placeholder="أكتب تفاصيل وشرح الدرس هنا..."
          ...
          <div className="flex items-center gap-3 p-3.5 rounded-xl bg-emerald-50/50 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-700/30">
            ...
            />
            <label htmlFor="create-is-preview" className="text-xs font-bold text-emerald-800 dark:text-emerald-300 cursor-pointer">
