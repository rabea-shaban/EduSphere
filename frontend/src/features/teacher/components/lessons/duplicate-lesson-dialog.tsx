...
import { useDuplicateLesson } from "@/hooks/useLessons";
...
export function DuplicateLessonDialog({ sectionId, lesson, isOpen, onClose }: DuplicateLessonDialogProps) {
  const duplicateLesson = useDuplicateLesson(sectionId);

  const handleConfirm = async () => {
    if (!lesson) return;
    await duplicateLesson.mutateAsync(lesson._id);
    onClose();
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
      ...
      <div className="relative w-full max-w-md bg-white dark:bg-[#0B2D5B] rounded-3xl shadow-2xl border border-slate-200 dark:border-white/10 p-6 space-y-5 text-right">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-base font-black text-[#0B2D5B] dark:text-white">تكرار الدرس</h2>
            <p className="text-xs text-slate-500 dark:text-slate-400">سيتم إنشاء نسخة من هذا الدرس كمسودة</p>
          </div>
        </div>

        <div className="p-4 rounded-2xl bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 space-y-1">
          <p className="text-xs font-bold text-slate-500">الدرس المراد تكراره:</p>
          <p className="text-sm font-black text-[#0B2D5B] dark:text-white">{lesson.title}</p>
        </div>
        ...
        <div className="flex items-center gap-3 pt-1">
          ...
          >
            {duplicateLesson.isPending ? (
              <Loader2 className="h-4 w-4 animate-spin" />
            ) : (
              <Copy className="h-4 w-4" />
            )}
            <span>{duplicateLesson.isPending ? "جاري التكرار..." : "تكرار الدرس"}</span>
          </button>
        </div>
      </div>
    </div>
...
export default DuplicateLessonDialog;
