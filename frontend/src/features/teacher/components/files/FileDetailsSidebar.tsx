...
import { X, Copy, Check, Edit2, Save, Folder, HardDrive, Download, Calendar, Tag } from "lucide-react";
...
export function FileDetailsSidebar({ file, onClose, onUpdate, isUpdating }: FileDetailsSidebarProps) {
  ...
  return (
    <div className="fixed inset-y-0 left-0 z-50 w-full sm:w-96 bg-white dark:bg-[#0F274D] border-r border-slate-200 dark:border-white/10 shadow-2xl p-6 overflow-y-auto space-y-6 text-right" dir="rtl">
      {/* Header */}
      ...
      <form onSubmit={handleSave} className="space-y-4">
        <div className="space-y-2">
          <label className="text-xs font-bold text-slate-700 dark:text-slate-200">اسم الملف الأصل</label>
          <input
            type="text"
            value={originalName}
        ...
        <div className="space-y-2">
          <label className="text-xs font-bold text-slate-700 dark:text-slate-200">اسم المجلد</label>
          <select
            value={folder}
          ...
          >
            ...
            <option value="assignments">assignments (الواجبات)</option>
          </select>
        </div>
        ...
        <button
          type="submit"
          disabled={isUpdating}
          className="w-full h-10 rounded-xl bg-[#0B2D5B] dark:bg-[#1E73D8] text-white text-xs font-bold shadow-md transition-all flex items-center justify-center gap-2"
        >
      ...
      <div className="space-y-3 pt-4 border-t border-slate-100 dark:border-white/10 text-xs">
        <h4 className="font-bold text-slate-800 dark:text-slate-200">الخصائص التقنية</h4>
        ...
        <div className="flex items-center justify-between p-3 rounded-xl bg-slate-50 dark:bg-white/5">
          <span className="text-slate-500">عدد مرات التحميل:</span>
      ...
      <div className="p-4 rounded-2xl bg-slate-50 dark:bg-white/5 border border-slate-200/60 dark:border-white/10 space-y-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="p-2 rounded-lg bg-[#F58220] hover:bg-[#e57310] text-white transition-all shadow-sm"
          >
