...
import { ConversationItem, ChatParticipant } from "@/types/chat";
import chatService from "@/services/chat.service";
import { Plus, Search, MessageSquare, GraduationCap, ShieldCheck, UserCheck, UserPlus, Loader2 } from "lucide-react";
...
export const ConversationSidebar: React.FC<ConversationSidebarProps> = ({
  ...
  const [searchedUsers, setSearchedUsers] = React.useState<ChatParticipant[]>([]);
  const [isSearchingUsers, setIsSearchingUsers] = React.useState(false);
  const [creatingChatUserId, setCreatingChatUserId] = React.useState<string | null>(null);
  ...
  React.useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        const users = await chatService.getAssignableUsers(roleParam, searchQuery.trim());
  ...
  const handleStartChatWithUser = async (user: ChatParticipant) => {
    try {
      setCreatingChatUserId(user._id);
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-3">
        ...
        {filteredConversations.length > 0 && (
          <div className="space-y-1">
            {searchQuery.trim() && (
              <span className="px-3 text-[11px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider block mb-1">
                المحادثات الحالية
              </span>
            )}
            {filteredConversations.map((conv) => {
              ...
              const lastText = lastMsgObj?.message || (lastMsgObj?.attachments?.length ? "مرفق صوتي/وسائط" : "ابدأ المحادثة...");
        ...
        {/* Platform Users Search Stream (When Searching) */}
        {searchQuery.trim() && (
          <div className="space-y-1 pt-2 border-t border-slate-200 dark:border-[#243047]">
            <div className="flex items-center justify-between px-3 mb-1">
              <span className="text-[11px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">
                مستخدمين على المنصة
              </span>
              {isSearchingUsers && <Loader2 className="w-3.5 h-3.5 text-[#1769D3] animate-spin" />}
            </div>

            {isSearchingUsers ? (
              <div className="flex items-center justify-center py-6 text-slate-400 text-xs gap-2">
                <Loader2 className="w-4 h-4 animate-spin text-[#1769D3]" />
                <span>جاري البحث عن مستخدمين...</span>
              </div>
            ) : searchedUsers.length === 0 ? (
              <div className="text-center py-4 px-2 text-slate-400 dark:text-slate-500 text-xs">
                لم يتم العثور على مستخدمين جدد بهذا الاسم
              </div>
            ) : (
              searchedUsers.map((u) => {
                const uName = `${u.firstName || ""} ${u.lastName || ""}`.trim() || u.username || "مستخدم";
                const isCreatingThis = creatingChatUserId === u._id;

                return (
                  <div
                    key={u._id}
                    onClick={() => !isCreatingThis && handleStartChatWithUser(u)}
                    className="flex items-center justify-between p-3 rounded-2xl hover:bg-slate-200/60 dark:hover:bg-[#172033]/70 cursor-pointer transition-all border border-slate-200/60 dark:border-white/5 bg-white dark:bg-[#0F172A]"
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="w-9 h-9 rounded-full bg-gradient-to-br from-indigo-500 to-blue-600 flex items-center justify-center text-white font-bold text-xs overflow-hidden shrink-0">
                        {u.avatar ? <img src={u.avatar} alt={uName} className="w-full h-full object-cover" /> : uName[0]}
                      </div>
                      <div className="flex flex-col min-w-0">
                        <div className="flex items-center gap-1.5 truncate">
                          <span className="text-xs font-semibold text-slate-900 dark:text-slate-100 truncate">{uName}</span>
                          {getRoleBadge(u.role)}
                        </div>
                        <span className="text-[10px] text-slate-400 dark:text-slate-500 truncate">
                          انقر لبدء محادثة فورية
                        </span>
                      </div>
                    </div>
                    ...
                    <button
                      ...
                      className="p-2 text-[#1769D3] hover:bg-[#1769D3]/10 rounded-xl transition-colors shrink-0"
                      title="محادثة جديدة"
                    >
                      {isCreatingThis ? <Loader2 className="w-4 h-4 animate-spin" /> : <UserPlus className="w-4 h-4" />}
                    </button>
                  </div>
                );
              })
        ...
        {/* Empty State when no results in both */}
        {filteredConversations.length === 0 && !searchQuery.trim() && (
          <div className="flex flex-col items-center justify-center py-16 px-4 text-center text-slate-400 dark:text-slate-500 text-xs space-y-3">
            <MessageSquare className="w-10 h-10 opacity-30" />
            <span>لا توجد محادثات سابقة</span>
            <button
              onClick={onOpenNewChatModal}
              className="mt-2 inline-flex items-center gap-1.5 px-4 py-2 bg-[#1769D3] hover:bg-blue-700 text-white rounded-xl text-xs font-semibold transition-all shadow-sm"
            >
              <Plus className="w-4 h-4" />
              <span>البحث عن شخص جديد</span>
            </button>
          </div>
        )}
      </div>
